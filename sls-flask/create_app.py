...
def init_auth(app: App) -> None:
    ...
    def user_loader_callback(identity):
        from sls-flask.model.user import User
...
def configure_class(app: App) -> None:
    ...
    config_class = os.getenv("sls-flask_CONFIG".upper())
    ...
    if not config_class:
        ...
        if stage:
            ...
            if stage == "prd":
                config_class = "sls-flask.config.ProductionConfig"
            else:
                config_class = "sls-flask.config.DevConfig"
        else:
            # local dev
            config_class = "sls-flask.config.LocalDevConfig"
...
def init_xray(app: App) -> None:
    ...
    xray_recorder.configure(service="sls-flask")
